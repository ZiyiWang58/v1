// Ziyi Wang Oct 2025 - v2 - MQTT messager to vespera with temperature and touch sensors (switch)
// Works with MKR1010
...
bool lightOn = true;        // Record the state
...
void loop() {
  ...
  int touchState = digitalRead(TOUCH_PIN);  // Read touch

  if (touchState == HIGH && lastTouchState == LOW) {
    lightOn = !lightOn;  // Switch the state
    Serial.print("Touch. Light: ");
    Serial.println(lightOn ? "ON" : "OFF");
    delay(300);
  }
  lastTouchState = touchState;  // Record the current state
